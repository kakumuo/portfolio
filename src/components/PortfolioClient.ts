...
export class PortfolioClient {
    ...
    private async getChangelogHelper({...args}:{owner:string, repo:string, path?:string}):Promise<PortfolioClientResponse<GitRevision[]>>{
        try {
            const resp = await this.octokit.rest.repos.getCommit({...args, per_page: 5} as any);
            ...
            for(let cur of respArr) {
                ...
                console.log(cur.commit.message.split("\n"))

                let curCommit:GitRevision = {} as GitRevision; 
                curCommit.id = cur.sha; 
                curCommit.body = cur.commit.message.split("\n").filter((_, i) => i > 0).join("\n"); ; 
                curCommit.url = cur.html_url; 
                curCommit.title = cur.commit.message.split("\n")[0]; 
                curCommit.date = 0; 
    ...
    async getPostChangelog({id}:{id:string}):Promise<PortfolioClientResponse<GitRevision[]>>{
        return this.getChangelogHelper({owner: OWNER, repo: BLOG_REPO, path: `posts/${id}`})
    }
    ...
    async getProjectChangelog({linkOrId}:{linkOrId:string}):Promise<PortfolioClientResponse<GitRevision[]>>{
        try {
            ...
            return this.getChangelogHelper({owner, repo})
        }catch(e:any) {
            return {
                success: false, 
                errorCode: 404, 
